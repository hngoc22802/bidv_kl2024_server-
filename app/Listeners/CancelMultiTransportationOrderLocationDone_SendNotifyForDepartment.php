...
class CancelMultiTransportationOrderLocationDone_SendNotifyForDepartment
{
    ...
    public function handle(CancelMultiTransportationOrderLocationDone $event): void
    {
        ...
        $query->whereHas('location', function ($query) use ($order_location_ids) {
            $query->whereIn('id', $order_location_ids);
        });
        $query->with('location.transportOrder.shift', 'location.transportOrder.request.type');
